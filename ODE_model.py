...
p.add_argument('-E_max', type=float,
...
p.add_argument("-use_diffusion", action='store_true',
               help='Include diffusion constant to the models')
p.add_argument('-dif_table', type=str,
                help='File with electron diffusion coefficient D')
#> ===================================================================
args = p.parse_args()
...
#> ===================================================================
TD_D = np.loadtxt(args.dif_table, skiprows=2).T
#> ===================================================================
...
if args.use_diffusion:
    ...
    if args.ni_bc is not None:
        y0[[i_ne, i_ni, i_q, i_w]] = [args.ne_bc, args.ni_bc, args.Q, 0.0]
    else:
        y0[[i_ne, i_ni, i_q, i_w]] = [args.ne_bc, args.ne_bc, args.Q, 0.0]
else:
    y0 = np.zeros(3)
    if args.ni_bc is not None:
...
def get_z_crit(Q, E_bg):
    ...
    if args.R is not None and args.E_max is None:
        return args.R
    else:
...
def ODE_model_rhs(z, y, I_ph, R):
    ...
    dq_dz = rho_eps * z**2
    dE_dz = get_dE_dz(z, q, dq_dz, args.E_bg)
    dmu_dz = dmu_dE * dE_dz
    dD_dz = dD_dE * dE_dz

    # d/dz [ne, ni, q]
    if args.neg_streamer and not args.use_diffusion:
        # Negative streamers
        dy[i_ne] = -1 / (args.v - mu * E) * (src - dmu_dz * E * ne -
                                             mu * rho_eps * ne) 
        dy[i_ni] = -src / args.v
        dy[i_q] = dq_dz
    elif not args.neg_streamer and not args.use_diffusion:
        # Postive streamers
        dy[i_ne] = -1 / (args.v + mu * E) * (src + dmu_dz * E * ne +
                                             mu * rho_eps * ne)
        dy[i_ni] = -src / args.v 
...
def residual_v_Q(x, y0):
    ...
    R = get_radius(sol)
    E_max = get_E_max(sol)
    residual = [(R - args.R) / R0_scale, (E_max - args.E_max) / E_max_scale]
    return residual

def check_convergence(sol):
    if np.abs(sol.fun).max() > 1e-2: #9e-2: # --> Aangepast
        sys.stderr.write(str(sol) + '\n' + str(args))
        raise ValueError('No convergence, adjust numerical parameters')
    return


if args.R is not None and args.E_max is not None:
    print("Solving for v")
    ...
    if args.fixed_I_ph is None:
        ...
        for Q in Q_guesses:
            ...
            tmp = root(residual_photoi_E_max, x0=(1., 5e-4, y0[i_q]),
                       args=(y0, ), method='lm', 
                       tol=args.rtol,
                       options={'eps': 5e-3}) #options={'eps': 5e-3}
            if np.abs(tmp.fun).max() < 1e-2:
                break
        check_convergence(tmp)
        ...
        # Fixed photoionization source, determine Q
        #> ##################################################
        #a, b = 5e-2*args.Q, 5e-2*args.Q # --> Dit werkt echt matig adhoc
        a, b = 30e1*args.Q, 9e-1*args.Q
        print(residual_E_max(a, y0, args.fixed_I_ph, 0.), residual_E_max(b, y0, args.fixed_I_ph, 0.))
        #> ##################################################
...
if args.compact_output:
    ...
    if args.neg_streamer and args.use_diffusion:
        print(f'Simulate a negative streamer with diffusion:')
    elif args.neg_streamer and not args.use_diffusion:
        print(f'Simulate a negative streamer without diffusion:')
    elif not args.neg_streamer and args.use_diffusion:
        print(f'Simulate a postive streamer with diffusion:')
    else:
        print(f'Simulate a postive streamer without diffusion:')
